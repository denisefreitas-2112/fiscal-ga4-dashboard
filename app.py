...
CANAL_ANALISTA = {
    "Conteúdo / Blog":    "Analista de Conteúdo",
    "E-mail / Automação": "Analista de Automação",
    "Google Ads":         "Analista de Mídias Digitais",
    "Meta Ads":           "Analista de Mídias Digitais",
    "Instagram":          "Analista de Mídias Digitais",
    "LinkedIn":           "Analista de Mídias Digitais",
    "Social":             "Analista de Mídias Digitais",
}

CORES_CANAL = {
    "Conteúdo / Blog":    "#059669",
    "E-mail / Automação": "#7c3aed",
    "Google Ads":         "#1a56db",
    "Meta Ads":           "#1877f2",
    "Instagram":          "#e1306c",
    "LinkedIn":           "#0077b5",
...
st.sidebar.markdown("---")
st.sidebar.caption(f"GA4 Property 307883096 · Jan–{'Dez' if str(ano) < str(date.today().year) else date.today().strftime('%b')}/{str(ano)[2:]}")

# ── Header ────────────────────────────────────────────────────────────────────
st.title("📊 Painel de Desempenho — Marketing Fiscal.IO")
st.caption(
    "Canais: Blog · E-mail / Zoho · Google Ads · Meta · Instagram · LinkedIn · Social"
    f" &nbsp;|&nbsp; Solicitado por Lucas Farley &nbsp;|&nbsp; {ano}"
)
st.markdown("---")
...
try:
    ...
    # ── VISÃO GERAL ───────────────────────────────────────────────────────────
    st.subheader(f"Visão geral · sessões por canal · {ano}")
    ...
    fig_geral = px.bar(
        ...
        labels={"mes": "", "sessions": "Sessões", "canal": "Canal"},
        color_discrete_map=CORES_CANAL,
        title="Sessões totais por canal",
    )
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    st.markdown('<div class="section-header">✍️ Analista de Conteúdo — Blog (conteudo.fiscal.io)</div>', unsafe_allow_html=True)

    df_blog = df_s[df_s["canal"] == "Conteúdo / Blog"]
    ...
    leads_blog    = df_l[df_l["canal"] == "Conteúdo / Blog"]["eventCount"].sum()
    dl_blog       = df_dl[df_dl["canal"] == "Conteúdo / Blog"]["eventCount"].sum()

    if not df_blog_mes.empty:
        cb1, cb2, cb3, cb4, cb5 = st.columns(5)
        cb1.metric("Sessões totais", f"{df_blog_mes['sessions'].sum():,}".replace(",", "."))
        cb2.metric("Sessões engajadas", f"{df_blog_mes['engagedSessions'].sum():,}".replace(",", "."))
        avg_eng = df_blog_mes["engRate"].mean() * 100
        cb3.metric("Engajamento médio", f"{avg_eng:.1f}%")
        avg_d = df_blog_mes["avgDur"].mean()
        cb4.metric("Duração média", f"{int(avg_d // 60)}m {int(avg_d % 60)}s")
        cb5.metric("Leads via Blog", f"{leads_blog:,}".replace(",", "."))

        col_b1, col_b2 = st.columns(2)
        with col_b1:
            fig_bs = px.line(df_blog_mes, x="mes", y="sessions",
                title="Sessões via Blog — mês a mês",
                category_orders={"mes": meses_label},
                labels={"mes": "", "sessions": "Sessões"},
        ...
        df_blog_camp = (
            ...
            .head(20)
            .rename(columns={"sessionCampaignName": "Artigo / Campanha", "sessions": "Sessões"})
        )
        ...
        st.info("Sem dados de Blog no período.")
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    st.markdown('<div class="section-header">📧 Analista de Automação — E-mail / Zoho Marketing Hub</div>', unsafe_allow_html=True)

    df_email = df_s[df_s["canal"] == "E-mail / Automação"]
    ...
    leads_email = df_l[df_l["canal"] == "E-mail / Automação"]["eventCount"].sum()
    dl_email    = df_dl[df_dl["canal"] == "E-mail / Automação"]["eventCount"].sum()

    if not df_email_mes.empty:
        ce1, ce2, ce3, ce4 = st.columns(4)
        ce1.metric("Sessões totais", f"{df_email_mes['sessions'].sum():,}".replace(",", "."))
        ce2.metric("Sessões engajadas", f"{df_email_mes['engagedSessions'].sum():,}".replace(",", "."))
        ...
        with col_em1:
            fig_es = px.bar(df_email_mes, x="mes", y="sessions",
                title="Sessões via E-mail — mês a mês",
                category_orders={"mes": meses_label},
                labels={"mes": "", "sessions": "Sessões"},
        ...
        with col_em2:
            df_l_email_mes = (
                df_l[df_l["canal"] == "E-mail / Automação"]
            ...
            if not df_l_email_mes.empty:
                fig_el = px.bar(df_l_email_mes, x="mes", y="eventCount",
                    title="Leads gerados via E-mail — mês a mês",
                    category_orders={"mes": meses_label},
                    labels={"mes": "", "eventCount": "Leads"},
                    color_discrete_sequence=["#7c3aed"])
        ...
        df_email_camp = (
            ...
            .sort_values("sessions", ascending=False)
            .head(20)
            .rename(columns={"sessionCampaignName": "Campanha / E-mail", "sessions": "Sessões"})
        )
        ...
        st.info("Sem dados de e-mail no período.")
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    st.markdown('<div class="section-header">📣 Analista de Mídias Digitais — Tráfego Pago e Social</div>', unsafe_allow_html=True)
    ...
    if not df_midia_mes.empty:
        ...
        cm1.metric("Sessões totais (mídia)", f"{df_midia_mes['sessions'].sum():,}".replace(",", "."))
        cm2.metric("Sessões engajadas", f"{df_midia_mes['engagedSessions'].sum():,}".replace(",", "."))
        ...
        fig_midia_stack = px.bar(
            ...
            barmode="stack",
            title="Sessões por canal de mídia — mês a mês",
            category_orders={"mes": meses_label},
            labels={"mes": "", "sessions": "Sessões", "canal": "Canal"},
            color_discrete_map=CORES_CANAL,
        )
        ...
        if not df_l_midia_mes.empty:
            fig_l_midia = px.bar(df_l_midia_mes, x="mes", y="eventCount", color="canal",
                barmode="group",
                title="Leads gerados por canal de mídia",
                category_orders={"mes": meses_label},
        ...
        df_midia_camp = (
            ...
            .sort_values("sessions", ascending=False)
            .head(20)
            .rename(columns={"canal": "Canal", "sessionCampaignName": "Campanha", "sessions": "Sessões"})
        )
        if not df_midia_camp.empty:
            st.caption("Top campanhas de mídia paga e social")
            st.dataframe(df_midia_camp, use_container_width=True, hide_index=True)
    else:
        st.info("Sem dados de mídia paga/social no período.")

except Exception as e:
    st.error(f"Erro ao conectar ao GA4: {e}")
    st.info("Verifique se os secrets estão configurados no Streamlit Cloud.")
